...
int MMUStart() {
	...
	while (TRUE) { // continue to read input until EOF
		if (Input_GetLine(&line) < 1) { // allocate memory for each line
			printf("End of File.\n");
			if(EXTRA_DEBUG)printf("MPT AFTER INSTRUCTIONS:\n");
			if(EXTRA_DEBUG)printMPT();
			if(EXTRA_DEBUG)printf("\nMAIN MEMORY AFTER INSTRUCTIONS:\n");
			if(EXTRA_DEBUG)printMainMem();
			if(EXTRA_DEBUG)printf("\nSECONDARY MEMORY AFTER INSTRUCTIONS:\n");
			if(EXTRA_DEBUG)printSecondMem();
			return 0;
		} else {
